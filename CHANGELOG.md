...
- 77e16b6: Add a `mfa` resource (`status`, `setup`, `enable`, `verify`, `regenerateRecoveryCodes`) for mandatory TOTP + recovery-code MFA, and a `userSessionToken` client option plus `stepUpToken` per-request option so mutating calls can carry the per-user session and step-up headers (`X-User-Session`, `X-Step-Up-Token`).
